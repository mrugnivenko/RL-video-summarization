...
parser.add_argument('--is-sup', default=0, help="whether to save output results")
...
def main():
    ...
    if args.is_sup:
        print("Training superwised method")
    else:
        print("Training unsuperwised method")
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        for idx in idxs:
            ...
            labels = dataset[key]['user_summary'][...]
            # print(seq)
            seq = torch.from_numpy(seq).unsqueeze(0) # input shape (1, seq_len, dim)
            if use_gpu: seq = seq.cuda()
            probs = model(seq.float()) # output shape (1, seq_len, 1)

            epis_rewards = []
            if args.is_sup:
                cost = torch.tensor(0.0)
                for user_label in labels:
                    ult = torch.tensor([[0,1] if x == 1 else [1,0] for x in user_label])[::15][:probs.shape[1]]
                    ult = ult.float()
                    
                    probs = probs.squeeze(0)[:ult.shape[0]]
                    probs = probs.float()
                    # print(ult, probs)
                    lossos = loss(input = probs, target = ult)
                    # print("выолютволфывфыолвфлфывюырвглыфрвлюоыЖ", lossos)
                    cost += lossos
                cost = cost / labels.shape[0]
                cost += args.beta * (probs.mean() - 0.5)**2
                epis_rewards.append(cost.item()) 
            else:
                probs = torch.softmax(model(seq), axis=-1).T[1]
            ...
            reward_writers[key].append(epis_rewards[0] if args.is_sup else np.mean(epis_rewards))
...
def evaluate(model, dataset, test_keys, use_gpu):
    ...
    with torch.no_grad():
        ...
        for key_idx, key in enumerate(test_keys):
            ...
            if use_gpu: seq = seq.cuda()
            probs = torch.softmax(model(seq.float()), axis=-1).T[1]
            probs = probs.data.cpu().squeeze().numpy()
            cps = dataset[key]['change_points'][...]
            num_frames = int(dataset[key]['n_frames'][()])
            nfps = dataset[key]['n_frame_per_seg'][...].astype(np.int32).tolist()
            positions = dataset[key]['picks'][...]
            try:
                user_summary = dataset[key]['user_summary'][...]

                machine_summary = vsum_tools.generate_summary(probs, cps, num_frames, nfps, positions)
                # comment to evaluate without labels
