...
class Generate_Dataset:
    ...
    def _get_change_points(self, video_feat, n_frame, fps):
        ...
        each_m = int(fps * 2)
        change_points = np.array(list(range(len(video_feat))))[::each_m]
        if change_points[0] != 0:
            change_points = np.concatenate(([0], change_points))
        if change_points[-1]!=len(video_feat)-1:
            change_points = np.concatenate((change_points,[len(video_feat)-1]))
        # K = np.dot(video_feat, video_feat.T)
#         algo = rpt.Dynp(model="l2").fit(video_feat)
#         change_points = algo.predict(n_bkps=m)
#         change_points = np.concatenate(([0], change_points))
    ...
    def generate_dataset(self):
        for video_idx, video_filename in enumerate(tqdm(self.video_list)):
            ...
            for frame_idx in tqdm(range(n_frames-1)):
                ...
                if success:
                    frame_feat = self._extract_feature(frame) #не процесить каждый кадр
            ...
            # print("Our points", change_points)
            # h5 = h5py.File('/Users/misha/Documents/Courses/Skoltech courses/RL_project/pytorch-vsumm-reinforce/datasets/eccv16_dataset_tvsum_google_pool5.h5', 'r')
            # keys = h5.keys()
            # print("Their points",)
            # for key in keys:
            #     cp = h5[key]['change_points'][...]
            #     print(f'{key}: {}')
...
if __name__ == "__main__":
    gen = Generate_Dataset('/Users/misha/Documents/Courses/Skoltech courses/RL_project/pytorch-vsumm-reinforce/datasets/snapememories.mp4', 'harry_potter.h5')
